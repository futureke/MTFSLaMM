...
tf.compat.v1.logging.set_verbosity(tf.compat.v1.logging.ERROR) # ignore data loading warnings
...
##########################################################
if args.model == "vCLUB":
    with tf.compat.v1.variable_scope('mi'):
        mu, logvar = mi(images)
        var = tf.exp(logvar)
...
######################## Loss ############################
##########################################################
with tf.compat.v1.variable_scope('decoder', reuse=True):
